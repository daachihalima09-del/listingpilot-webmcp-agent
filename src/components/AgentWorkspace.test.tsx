...
vi.mock('@/webmcp/register-tools', () => ({
  registerListingPilotTools: () => ({ supported: false, ready: Promise.resolve(), unregister: vi.fn() }),
}));
...
describe('agent workspace proposal lifecycle', () => {
  it('shows approved waiting state and then the persisted published state', async () => {
    ...
    vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({ activity: [], latestProposal: approved, publishedProducts: [] }), { status: 200, headers: { 'content-type': 'application/json' } })));

...
import type { AuditEvent, ListingProposal, ProductInspection, ProductSummary } from '@/domain/contracts';
...
export function AgentWorkspace({ initialProducts, initialInspection }: { initialProducts: ProductSummary[]; initialInspection: ProductInspection }) {
  ...
  const loadActivity = useCallback(async () => {
    const body = await jsonRequest<{ activity: AuditEvent[] }>('/api/activity');
  ...
  return <main>
    <header className="topbar">
      ...
      <div className="agent-status" aria-live="polite">
        ...
        <span><strong>{webMcpState === 'ready' ? 'Agent tools ready' : webMcpState === 'unsupported' ? 'WebMCP preview' : webMcpState === 'error' ? 'Tool registration failed' : 'Checking WebMCP'}</strong><small>{webMcpState === 'ready' ? '3 scoped tools · no publish access' : webMcpState === 'unsupported' ? 'Use ChatGPT Browser or enabled Chrome' : 'Secure registration lifecycle'}</small></span>
      </div>
    </header>

    <section className="journey" aria-label="Safe collaboration workflow">
      <span className="journey-active">1 Search</span><i>→</i><span className="journey-active">2 Inspect truth</span><i>→</i><span className="journey-active">3 Prepare</span><i>→</i><span>4 Human reviews</span><i>→</i><span className="journey-locked">Publish locked</span>
    </section>
    ...
    <div className="workspace-grid">
      ...
      <aside className="panel proposal-panel" aria-labelledby="proposal-title">
        <div className="panel-heading"><div><p className="eyebrow">PROPOSED CHANGES</p><h2 id="proposal-title">Human review queue</h2></div>{proposal && <StatusPill tone={proposal.status === 'APPROVED' ? 'good' : 'warn'}>{proposal.status.replace('_', ' ')}</StatusPill>}</div>
        {!proposal ? <div className="empty-proposal"><div className="empty-icon">✦</div><h3>No proposal yet</h3><p>The agent can prepare improvements from verified facts. It cannot approve or publish them.</p><button className="primary" onClick={() => void prepareProposal()} disabled={busy !== null}>{busy === 'prepare' ? 'Preparing…' : 'Prepare safe improvement'}</button></div> : <div className="proposal-content">
    ...
    <footer><span>Challenge workspace: Atlas Demo</span><span>Synthetic data only · no OpenAI · no Shopify · no merchant credentials</span></footer>
  </main>;
}
